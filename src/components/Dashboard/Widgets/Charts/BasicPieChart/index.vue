...
const WIDGET_NAME = 'PieRose'
const type1 = [
  { flag: 'PieBasic', styleName: 'PieBasicStyle', icon: require('./icon/thumb-piebasicchart.png'), title: '环形图', desc: '基础环形图' }
  // { flag: 'PieRose', icon: require('./icon/thumb-pierosechart.png'), title: '玫瑰图', desc: '玫瑰图' }
]
export default {
  type: type1,
  name: WIDGET_NAME,
  group: 'chart',
  icon: require('./icon/thumb-pierosechart.png'),
  title: '玫瑰图',
  panel: stylec,
  setting: {
    type: 'PieRose',
  ...
  computed: {
    ...
    options () {
      return {
        ...
        series: [{
          name: this.val.csvNum || '销量',
          type: 'pie',
          radius: this.val.seriesRadius,
          roseType: this.val.type === 'PieBasic' ? '' : 'area',
          center: ['50%', '40%'],
  ...
  mounted () {
    ...
    if (this.val.type === 'PieBasic') {
      let param = [
        { name: 'toPie', value: true },
        { name: 'seriesRadius', value: ['45%', '60%'] },
