<template>
  <div
    ...
    <div
      ...
      <div
        v-for="(head, key, index) in newTableData[0]"
        :key="index"
        :head="head"
        :index="index"
        class="table-th"
        :style="{
          ...
          borderRight: index === newTableData.length-1 ? '' : val.tbBorderRgiht
        }"><span>{{key}}</span></div>
    </div>
    ...
    <div
      ...
      <!-- 表内容块 -->
      <div
        v-for="(item, index) in newTableData.slice(0, val.dataLength)"
        :key="index"
        class="table-tr" >
        <div
          v-show="val.showNum"
...
export default {
  ...
  setting: {
    ...
    csvSeries: [], // 分组标签(表格组件为数据，其他则为字符串)
    // csvNum: [], // 取值标签 (堆叠图为数组,表格组件无该属性，其他则为字符串)
    csvHeader: [], // 选中的表头关系
  ...
  computed: {
    newTableData () {
      ...
      if (this.dynamicData[this.val.keyPrimary] && this.dynamicData[this.val.keyPrimary][this.val.keyTarget]) {
        return this.changeTableDataType(this.dynamicData.data)
      } else {
        return []
      }
    }
  ...
  methods: {
    changeTableDataType (data) {
      let newData = []
      for (let i in data[this.val.keyTarget]) {
        let items = data[this.val.keyTarget][i]
        let newItem = {}
        for (let index in data.columns) {
          newItem[data.columns[index].name] = items[data.columns[index].id]
        }
